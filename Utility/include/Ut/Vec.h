...
namespace Ut
{
    ...
    public:
        ...
        // 访问器
        T& x() noexcept requires (N >= 1) { return data[0]; }
        T& y() noexcept requires (N >= 2) { return data[1]; }
        T& z() noexcept requires (N >= 3) { return data[2]; }
        const T& x() const noexcept requires (N >= 1) { return data[0]; }
        const T& y() const noexcept requires (N >= 2) { return data[1]; }
        const T& z() const noexcept requires (N >= 3) { return data[2]; }

        // 设置器
        void setX(T value) noexcept requires (N >= 1) { data[0] = value; }
        void setY(T value) noexcept requires (N >= 2) { data[1] = value; }
        void setZ(T value) noexcept requires (N >= 3) { data[2] = value; }

        // 运算符
        T& operator[](size_t index) noexcept { return at(index); }
        const T& operator[](size_t index) const noexcept { return at(index); }
